...
export async function awardExp(
  ...
  const prevLv   = calcLevel(players.exp)
  const newExp   = Math.max(0, players.exp + change)
  const newMonthExp = Math.max(0, players.month_exp + change)
...
export async function attendanceCheck(member: string) {
  ...
  if (dayOfWeek === 1) {
    prevDate.setDate(now.getDate() - 3) // 월요일이면 금요일
  } else {
